...
$.extend(true, systemDictionary, {
    ...
    strokeSmoothing: { en: 'Smoothing factor', de: 'Glättungsfaktor' },
...
vis.binds['vis-homekittiles'] = {
    ...
    infoText: function (widAttr, data) {
        ...
        if (data[1] === 'hktButtonDialogClose') {
            text = 'hktButtonDialogCloseDescription';
        } else if (data[1] === 'hktButtonNavigation') {
            text = 'hktButtonNavigationDescription';
        } else if (data[1] === 'hktButtonSet') {
            text = 'hktButtonSetDescription';
        } else if (data[1] === 'hktButtonSetNavigation') {
            text = 'hktButtonSetNavigationDescription';
        } else if (data[1] === 'hktButtonSetNavigationSubmenu') {
            text = 'hktButtonSetNavigationSubmenuDescription';
        } else if (data[1] === 'hktConfigItemModal') {
            text = 'hktConfigItemModalDescription';
        } else if (data[1] === 'hktConfigItemWaitscreen') {
            text = 'hktConfigItemWaitscreenDescription';
        } else if (data[1] === 'hktDatepicker') {
            text = 'hktDatepickerDescription';
        } else if (data[1] === 'hktHtmlDialog') {
            text = 'hktHtmlDialogDescription';
        } else if (data[1] === 'hktJsonTable') {
            text = 'hktJsonTableDescription';
        } else if (data[1] === 'hktLabel') {
            text = 'hktLabelDescription';
        } else if (data[1] === 'hktNotification') {
            text = 'hktNotificationDescription';
        } else if (data[1] === 'hktPinpadDialog') {
            text = 'hktPinpadDialogDescription';
        } else if (data[1] === 'hktRadiobuttons') {
            text = 'hktRadiobuttonsDescription';
        } else if (data[1] === 'hktSettingsBool') {
            text = 'hktSettingsBoolDescription';
        } else if (data[1] === 'hktSettingsSelect') {
            text = 'hktSettingsSelectDescription';
        } else if (data[1] === 'hktSettingsValue') {
            text = 'hktSettingsValueDescription';
        } else if (data[1] === 'hktSwitchBool') {
            text = 'hktSwitchBoolDescription';
        } else if (data[1] === 'hktThermostatDialog') {
            text = 'hktThermostatDialogDescription';
        } else if (data[1] === 'hktValue') {
            text = 'hktValueDescription';
        } else if (data[1] === 'hktViewInWidgetDialog') {
            text = 'hktViewInWidgetDialogDescription';
        } else if (data[1] === 'hktViewInWidgetSwipe') {
            text = 'hktViewInWidgetSwipeDescription';
    ...
    hktDatepicker: function (el, options) {
        ...
        if (!vis.editMode) {
            $this.hktDatepicker(datepickerOptions);
        }
    ...
    switch: function (el, data) {
        ...
        if (!trueVal || trueVal == '' || trueVal === 'true') {
            trueVal = true;
        }
        if (!falseVal || falseVal == '' || falseVal === 'false') {
            falseVal = false;
        }
        if (trueVal === 'false') {
            trueVal = false;
    ...
    dialogContainer: function (el, options, persistent, preload) {
        ...
        if (options.dialog_width !== undefined) {
            options.width = options.dialog_width.match(regexNumber);
        }
        if (options.dialog_height !== undefined) {
            options.height = options.dialog_height.match(regexNumber);
        }
        if (options.dialog_top !== undefined) {
            options.top = options.dialog_top.match(regexNumber);
        }
        if (options.dialog_left !== undefined) {
            options.left = options.dialog_left.match(regexNumber);
        }
            ...
            $.extend(
                {
                    ...
                    open: function () {
                        ...
                        $(this)
                            ...
                            .addClass('header')
                            .removeClass('ui-widget-header ui-corner-all ui-draggable-handle');
                        //title
                        ...
                        //set OID on open
                        if (options.setIdOnOpenClose && options.setValueOnOpen) {
                            vis.setValue(options.setIdOnOpenClose, options.setValueOnOpen);
                        }
                        ...
                        //position window
                        if (options.top || options.top === 0 || options.top === '0') {
                            $(this).parent().css('top', `${options.top}px`);
                        }
                        if (options.left || options.left === 0 || options.left === '0') {
                            $(this).parent().css('left', `${options.left}px`);
                        }

                        //scroll content
                        if (options.overflowX) {
                            $(this).css('overflow-x', options.overflowX);
                        }
                        if (options.overflowY) {
                            $(this).css('overflow-y', options.overflowY);
                        }

                        //arrows
                        if (options.arrowDirection) {
                            $(this).parent().addClass(`arrow-${options.arrowDirection}`);
                        }
                    ...
                    close: function () {
                        ...
                        //set OID on close
                        if (options.setIdOnOpenClose && options.setValueOnClose) {
                            vis.setValue(options.setIdOnOpenClose, options.setValueOnClose);
                        }
    ...
    formatValue: function (val, decimals, factor, comma, tdp) {
        ...
        if (!isNaN(parseFloat(val))) {
            ...
            if (factor !== undefined && factor !== '') {
                val = val * parseFloat(factor);
            }
            if (decimals !== undefined && decimals !== '') {
                val = val.toFixed(parseFloat(decimals, 10));
            }
    ...
    thermostatSlider: function (el, data, options) {
        ...
        $this.slider(settings).each(function () {
            // show steps
            ...
            for (let i = 0; i <= vals; i++) {
                ...
                label = $(`<span class="step-label">${val}</span>`).css('top', `${(i / vals) * 100}%`);
    ...
    dialogThermostatAutofill: function (widgetID, view, value, attr, isCss) {
        ...
        if (autofill && value) {
            ...
            if (oidPath[0] === 'hm-rpc' && oidPath[4] === 'ACTUAL_TEMPERATURE') {
                if (
                    vis.states.attr(`${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.1.SET_POINT_TEMPERATURE.val`) !==
                    undefined
                ) {
                    vis.views[view].widgets[widgetID].data.oidSetPointTemperature =
                        `${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.1.SET_POINT_TEMPERATURE`;
                }
                if (vis.states.attr(`${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.1.SET_POINT_MODE.val`) !== undefined) {
                    vis.views[view].widgets[widgetID].data.oidSetPointMode =
                        `${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.1.SET_POINT_MODE`;
                }
                if (vis.states.attr(`${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.1.HUMIDITY.val`) !== undefined) {
                    vis.views[view].widgets[widgetID].data.oidHumidity =
                        `${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.1.HUMIDITY`;
                }
                if (vis.states.attr(`${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.1.ACTIVE_PROFILE.val`) !== undefined) {
                    vis.views[view].widgets[widgetID].data.oidActiveProfile =
                        `${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.1.ACTIVE_PROFILE`;
                }
                if (vis.states.attr(`${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.0.LOW_BAT.val`) !== undefined) {
                    vis.views[view].widgets[widgetID].data.oidLowBat =
                        `${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.0.LOW_BAT`;
                }
                if (vis.states.attr(`${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.0.UNREACH.val`) !== undefined) {
                    vis.views[view].widgets[widgetID].data.oidUnreach =
                        `${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.0.UNREACH`;
                }
                if (vis.states.attr(`${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.1.WINDOW_STATE.val`) !== undefined) {
                    vis.views[view].widgets[widgetID].data.oidWindowState =
                        `${oidPath[0]}.${oidPath[1]}.${oidPath[2]}.1.WINDOW_STATE`;
                }
            }
    ...
    pinpadFunction: function (el, data) {
        ...
        function pinpad(event, value) {
            ...
            if (event == 'delete') {
                pinpadInput = pinpadInput.slice(0, -1);
            }
            ...
            if (event == 'enter' || (pinAutoEnter && pinpadInput.length == pin.length)) {
                if (pinpadInput == pin) {
                    ...
                    if (pinCorrectSetOid && pinCorrectSetOid != undefined && pinCorrectSetOid != '') {
                        vis.setValue(pinCorrectSetOid, true);
                    }
                    if (pinWrongSetOid && pinWrongSetOid != undefined && pinWrongSetOid != '') {
                        vis.setValue(pinWrongSetOid, false);
                    }
                    ...
                    if (pinCorrectSetOid && pinCorrectSetOid != undefined && pinCorrectSetOid != '') {
                        vis.setValue(pinCorrectSetOid, false);
                    }
                    if (pinWrongSetOid && pinWrongSetOid != undefined && pinWrongSetOid != '') {
                        vis.setValue(pinWrongSetOid, true);
                    }
                }
    ...
    jsonTableInsertRow: function (rowData, wid, options, rowNumber, noTR, index, serverID) {
        ...
        for (var obj in rowData) {
            ...
            if (!options.numberOfCols || k <= options.numberOfCols) {
                text += `<td class="row-item content ${options[`textalignCol${k}`]}${
                    options['noLineBreak'] ? ' nowrap' : ''
                }" ${options[`colWidth${k}`] ? `style="width:${options[`colWidth${k}`]}"` : ''}>${rowData[attr]}</td>`;
            }
            k++;
        }
    ...
    jsonTableCreateTable: function (view, wid, options) {
        ...
        var tableJson = options.oidJsonTable
            ? vis.states.attr(`${options.oidJsonTable}.val`)
            : '[{"Title": "first", "Value": 1, "Description": "Value1"}]';
        var table = [];
        ...
        var text = `<div class="table-content-wrapper${options.scrollable ? ' scrollable' : ''}${
            options.showHeader === 'never' ? ' noheader' : ''
        ...
        for (var ii = 0, ilen = table.length; ii < ilen; ii++) {
            ...
            if (!headerDone) {
                ...
                for (var obj in table[ii]) {
                    ...
                    if (!options.numberOfCols || k <= options.numberOfCols) {
                        ...
                        header +=
                            options['textalignHead'] != 'aligncontent'
                                ? options['textalignHead']
                                : options[`textalignCol${k}`];
                        header += `" ${
                            options[`colWidth${k}`] ? `style="width:${options[`colWidth${k}`]}"` : ''
                        }>${options[`colName${k}`] || attr}</th>`;
                    }
                    k++;
                }
                headerDone = true;
            }
            text += vis.binds['vis-homekittiles'].jsonTableInsertRow(
                table[ii],
                wid,
                options,
                j,
            ...
            j++;
            if (options.maxRows && j >= options.maxRows) {
                break;
            }
        }
        ...
        if (
            table.length === 0 &&
            (options.showHeader === 'ever' ||
                options.showHeader === 'everAndEmptyLine' ||
                options.showHeader === 'everAndNotice')
        ) {
            header += '<tr class="table-row head">';
                ...
                if (!options.numberOfCols || k <= options.numberOfCols) {
                    ...
                    header +=
                        options['textalignHead'] != 'aligncontent'
                            ? options['textalignHead']
                            : options[`textalignCol${k}`];
                    header += `" ${options[`colWidth${k}`] ? `style="width:${options[`colWidth${k}`]}"` : ''}>${
                        options[`colName${k}`]
                    }</th>`;
                }
            }
            ...
            if (options.showHeader === 'everAndEmptyLine') {
                ...
                for (var k = 1; k <= options.numberOfCols; k++) {
                    text += `<td class="row-item content" ${
                        options[`colWidth${k}`] ? `style="width:${options[`colWidth${k}`]}"` : ''
                    }>&nbsp;</td>`;
                }
                text += '</tr>';
            ...
            if (options.showHeader === 'everAndNotice') {
                text += '<tr class="table-row content">';
                text += `<td class="row-item content empty-table notice" style="width: 100%;">${
                    options.noticeEmptyTable
                }</td>`;
                text += '</tr>';
            }
        }
    ...
    jsonTableAutocomplete: function (widgetID, view, newId, attr, isCss) {
        ...
        setTimeout(function () {
            ...
            if (
                vis.views[view].widgets[widgetID].data.oidJsonTable !== undefined &&
            ...
            for (var i = 1; i <= numberOfCols; i++) {
                ...
                target.autocomplete('instance')._renderItem = function (ul, item) {
                    return $('<li>').attr('data-value', item.value).append(` ${item.value}`).appendTo(ul);
                };
            }
        }, 200);
        return ' ';
    ...
    addButtonIcon: function (el, data) {
        ...
        function icon(state) {
            //get state
            if (state == undefined) {
                state = $this.parent().parent().hasClass('state-active');
            }
            ...
            if (data.iconOn !== '' && data.iconOn !== undefined) {
                iconOn = true;
            }
            if (data.iconOff !== '' && data.iconOff !== undefined) {
                iconOff = true;
            }
            ...
            if (state) {
                if (iconOn) {
                    iconSrc = data.iconOn;
                } else if (icon && iconOff) {
                    iconSrc = data.icon;
                } //for compatibility if widget was placed when there was no iconOn/Off function
                else if (iconOff) {
                    iconSrc = data.iconOff;
                } else if (icon) {
                    iconSrc = data.icon;
                } else {
                    iconSrc = '';
    ...
    addLabelGroup1: function (el, data) {
        ...
        var val2 = vis.binds['vis-homekittiles'].formatValue(
            vis.states.attr(`${data.label2oid}.val`),
            data.label2decimals,
            data.label2factor,
            data.label2comma,
            data.label2tdp,
        ...
        if (data.label) {
            html += `<span class="label">${data.label}</span>`;
        }
        if (
            data.label2pre !== undefined ||
            data.label2oid !== undefined ||
            data.label2unit !== undefined ||
            data.label2post !== undefined
        ) {
            html += `<br>`;
        }
        if (data.label2pre) {
            html += `<span class="label2pre">${data.label2pre} </span>`;
        }
        if (data.label2oid) {
            html += `<span class="label2value">${val2}</span>`;
        }
        if (data.label2unit) {
            html += `<span class="label2unit">${data.label2unit}</span>`;
        }
        if (data.label2post) {
            html += `<span class="label2post"> ${data.label2post}</span>`;
        }
        ...
        if (data.label2oid) {
            vis.states.bind(`${data.label2oid}.val`, function (e, newVal, oldVal) {
                var value = vis.binds['vis-homekittiles'].formatValue(
                    newVal,
                    data.label2decimals,
                    data.label2factor,
                    data.label2comma,
    ...
    addLabelGroup2: function (el, data) {
        ...
        var val3 = vis.binds['vis-homekittiles'].formatValue(
            vis.states.attr(`${data.label3oid}.val`),
            data.label3decimals,
            data.label3factor,
            data.label3comma,
            data.label3tdp,
        );
        var val4 = vis.binds['vis-homekittiles'].formatValue(
            vis.states.attr(`${data.label4oid}.val`),
            data.label4decimals,
            data.label4factor,
            data.label4comma,
            data.label4tdp,
        );
        var val5 = vis.binds['vis-homekittiles'].formatValue(
            vis.states.attr(`${data.label5oid}.val`),
            data.label5decimals,
            data.label5factor,
            data.label5comma,
            data.label5tdp,
        ...
        if (data.label3pre) {
            html += `<span class="label3pre">${data.label3pre} </span>`;
        }
        if (data.label3oid) {
            html += `<span class="label3value">${val3}</span>`;
        }
        if (data.label3unit) {
            html += `<span class="label2unit">${data.label3unit}</span>`;
        }
        if (data.label3post) {
            html += `<span class="label3post"> ${data.label3post}</span>`;
        }
        if (
            data.label4pre !== undefined ||
            data.label4oid !== undefined ||
            data.label4unit !== undefined ||
            data.label4post !== undefined
        ) {
            html += `<br>`;
        }
        if (data.label4pre) {
            html += `<span class="label4pre">${data.label4pre} </span>`;
        }
        if (data.label4oid) {
            html += `<span class="label4value">${val4}</span>`;
        }
        if (data.label4unit) {
            html += `<span class="label4unit">${data.label4unit}</span>`;
        }
        if (data.label4post) {
            html += `<span class="label4post"> ${data.label4post}</span>`;
        }
        if (
            data.label5pre !== undefined ||
            data.label5oid !== undefined ||
            data.label5unit !== undefined ||
            data.label5post !== undefined
        ) {
            html += `<br>`;
        }
        if (data.label5pre) {
            html += `<span class="label5pre">${data.label5pre} </span>`;
        }
        if (data.label5oid) {
            html += `<span class="label5value">${val5}</span>`;
        }
        if (data.label5unit) {
            html += `<span class="label5unit">${data.label5unit}</span>`;
        }
        if (data.label5post) {
            html += `<span class="label5post"> ${data.label5post}</span>`;
        }
    ...
    addIncrement: function (el, data) {
        ...
        function showHideIncrement(show) {
            if (show) {
                ...
                if (data.incrementPlusShow || data.incrementMinusShow) {
                    ...
                    $this.html(html);
                    vis.binds['vis-homekittiles'].increment(
                        $this.parent().find('.incrementPlus'),
                        data.incrementValueMax,
                    );
                    vis.binds['vis-homekittiles'].increment(
                        $this.parent().find('.incrementMinus'),
                        data.incrementValueMin,
                    );
                }
            } else {
                html = '';
        ...
        if (
            vis.editMode ||
            !data.incrementShowOnlyIfTrue ||
            (data.incrementShowOnlyIfTrue && vis.states[`${data.oid}.val`])
        ) {
        ...
        if (data.oid) {
            vis.states.bind(`${data.oid}.val`, function (e, newVal, oldVal) {
                if (data.incrementShowOnlyIfTrue) {
                    showHideIncrement(newVal);
                }
            });
        }
        if (data.oidIncrement) {
            vis.states.bind(`${data.oidIncrement}.val`, function (e, newVal, oldVal) {
    ...
    addBlockOperation: function (el, data) {
        ...
        function blocked(blocked) {
            ...
            if (data.blockOperationShowIcon) {
                if ((vis.editMode && !data.blockOperationIconNotInEditor) || (!vis.editMode && blocked)) {
                    if (data.blockOperationIcon) {
                        html += `<img class="blockedIcon" src="${data.blockOperationIcon}">`;
                    }
                }
            }
        ...
        if (
            (data.blockOperationIfTrue && currentState == true) ||
            (data.blockOperationIfFalse && currentState != true)
        ) {
            blocked(true);
        } else {
            blocked(false);
        ...
        if (data.blockOperationIfTrue || data.blockOperationIfFalse) {
            function updateBlocked(stateValue) {
                if (
                    (data.blockOperationIfTrue && stateValue == true) ||
                    (data.blockOperationIfFalse && stateValue != true)
                ) {
                    blocked(true);
                } else {
                    blocked(false);
            ...
            if (data.blockOperationUseDifferentOID) {
                vis.states.bind(`${data.oidBlockOperation}.val`, function (e, newVal, oldVal) {
                    updateBlocked(newVal);
                });
            } else {
                vis.states.bind(`${data.oid}.val`, function (e, newVal, oldVal) {
                    updateBlocked(newVal);
    ...
    addSignalIcon: function (el, data) {
        ...
        function showHideSignal(value) {
            if (compVal && compVal === 'true') {
                compVal = true;
            }
            if (compVal && compVal === 'false') {
                compVal = false;
            }
